...
const rawEntries: Array<{ slpaCode: string; bankNameRaw: string; centralBankCode: string }> = [
  // Page 1
...
export const parseBankInfo = (raw: string) => {
...
const runSeeder = async () => {
  try {
    ...
    const records = rawEntries.map(entry => {
      const parsed = parseBankInfo(entry.bankNameRaw);
      return {
    ...
    await BankBranch.bulkCreate(records);
